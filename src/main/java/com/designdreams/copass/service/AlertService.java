...
import com.designdreams.copass.dao.AlertDAOImpl;
...
public class AlertService {
    ...
    public ResponseEntity<String> setupAlert(@RequestBody AlertRequest alertRequest,
        ...
        String status = null;
        String uuid = null;
        boolean alertEnableInd;

        try {

            logger.info(" Alert Request! " + alertRequest.toString());
            ...
            if (null != (respMsg = ResponseUtil.validate(alertRequest)))
...
//                return ResponseUtil.getResponse(HttpStatus.UNAUTHORIZED, "UNAUTHORIZED", "Login failed");
             ...
             tripId = alertRequest.getTripId();
             travellerId = alertRequest.getTravellerId();
             alertEnableInd = alertRequest.isAlertEnableIndicator();
             status = alertDAO.setupAlert(uuid,tripId,travellerId,alertEnableInd);

            if ("SUCCESS".equalsIgnoreCase(status)) {
                responseEntity = ResponseUtil.getResponse(HttpStatus.OK, "SUCCESS", "Successfully created Alert!");
            ...
            } else if (status.contains("DUPLICATE_ALERT")) {
                responseEntity = ResponseUtil.getResponse(HttpStatus.CONFLICT, status, "You have an Alert already on this SRC,DEST or DATE!");

            } else {
                responseEntity = ResponseUtil.getResponse(HttpStatus.INTERNAL_SERVER_ERROR, status, "Failed to create Alert!");

            }

        } catch (Exception e) {
            logger.error(e, e);
            responseEntity = ResponseUtil.getResponse(HttpStatus.INTERNAL_SERVER_ERROR, "FAILURE", "Error in creating alert!");
...
//                        .collect(Collectors.toList());
    ...
    @RequestMapping(
            ...
            consumes = MediaType.APPLICATION_JSON_VALUE,
            produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<String> disableAlert(
        ...
        try {
            ...
            logger.info(" diable alert  by user :: " + deleteTripRequest.toString());
    ...
    @RequestMapping(value = "/getAllAlertsByUser", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<String> getAllAlertsByUser(
